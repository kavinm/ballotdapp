...
function DelegateVotes() {
  ...
  return (
    <div>
      <input placeholder="input vote" onChange={(e) => setDelegateInput(e.target.value)}></input>
      <button onClick={() =>
        delegateVotes(signer, delegateInput, setLoading, setTxData)}>
        Delegate Votes</button>
    </div>
  )
}

function delegateVotes(setLoading, setTxData) {
  setLoading(true);
  const requestOptions = {
    method: 'POST',
    headers: {
  ...
  fetch('http://localhost:3000/delegatevote', requestOptions)
    .then(response => response.json())
    .then((data) => {
...
function Vote() {
  ...
  return (
    <div>
      <input placeholder="input vote" onChange={(e) => setVoteInput(e.target.value)}></input>
      <button onClick={() =>
        vote(signer, voteInput, setLoading, setTxData)}>
        Vote on Proposal</button>
    </div>
  )
}

function vote(setLoading, setTxData) {
  setLoading(true);
  const requestOptions = {
    method: 'POST',
    headers: {
  ...
  fetch('http://localhost:3000/vote', requestOptions)
    .then(response => response.json())
    .then((data) => {
      setTxData(data); setLoading(true);
    });
...
function ViewProposals() {
  ...
  // Add logic to view propsals
    {}

  if (isLoading) return <div>Fetching Proposals...</div>
  if (isError) return <div>error fetching proposals</div>
  return (
    <div>
      Proposals: {data?.formatted}
    </div>
  )
}