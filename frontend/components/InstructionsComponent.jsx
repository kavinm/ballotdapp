...
const initialProposals = [
  { name: "Proposal 1", votes: 5 },
  { name: "Proposal 2", votes: 10 },
];
...
function PageBody() {
  return (
    <div>
      ...
      <ProposalTable proposals={initialProposals}></ProposalTable>
    </div>
  );
}
  ...
  if (signer)
    return (
      <>
        ...
        <button onClick={() => signMessage(signer, "I love fried potatoes")}>
          Sign
        </button>
...
function RequestTokens() {
  ...
  return (
    <div>
      <button
        onClick={() => requestTokens(signer, "0.01", setLoading, setTxData)}>
        Request 1.0 MTK Token
      </button>
    </div>
  );
}
